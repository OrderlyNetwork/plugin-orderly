...
function formatPosition(position: API.Position): string {
  return `Symbol: ${position.symbol.split("_")[1]} - Side: ${
    ...
    position.mark_price
  } - Quantity: ${Math.abs(position.position_qty)} - Est. Liquidation Price: ${
    position.est_liq_price
  } - 24h pnl: ${position.pnl_24_h} - Created At: ${new Date(
    position.timestamp
  ).toLocaleString()}`;
}
...
const orderlyProvider: Provider = {
  get: async (
    ...
    try {
      ...
      const leverage = 1 / positionsAggregated.current_margin_ratio_with_orders;
